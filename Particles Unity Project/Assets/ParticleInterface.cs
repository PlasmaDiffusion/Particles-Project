...
//            m.startColor = new Color(g_Arr[i].Col.x, g_Arr[i].Col.y, g_Arr[i].Col.z);
    ...
    void Start()
    {
        ...
        //Debug hud stuff
        textHUD = GameObject.Find("VelocityText");
        velText = textHUD.GetComponent<Text>();
        ...
        //Instantiate other particle emitters. Also place them randomly around the scene.
        for (int i = 0; i < 50; i++)
        {
            ParticleArr[i] = Instantiate<GameObject>(particleObject);
            if (type == 1)ParticleArr[i].transform.position = new Vector3 (Random.Range(-80.0f, 80.0f), Random.Range(-20.0f, 20.0f), Random.Range(-80.0f, 80.0f) * i);
